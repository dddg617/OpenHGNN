...
class HGAT(BaseModel):
    ...
    .. math::
       a_{\tau} = \sigma(\mu_{\tau}^T \cdot [h_v \parallel h_{\tau}])  (1)
    
    The type embedding is :math:`\h_{\tau}=\sum_{v^'}\widetilde{A_{vv^'}}h_{v^'}`, 
    ...
    .. math::
       \alpha_{\tau} = \frac{exp(a_{\tau})}{\sum_{\tau^'\in \mathcal{T} exp(a_{\tau^'})}}  (2)
    ...
    .. math::
        b_{vv^'} = \sigma(\Mu^T \cdot \alpha_{\tau^'}[h_v \parallel h_{v^'}])  (3)
    
    where :math:`\Mu` is the attention vector.
    
    And the node-level attention weights is:
    
    .. math::
       \beta_{vv^'} = \frac{exp(b_{vv^'})}{\sum_{i\in \mathcal{N}_v exp(b_{vi})}}  (4)
    
    The final output is:
    
    .. math::
       H^{(l+1)} = \sigma(\sum_{\tau \in \mathcal{T}}\Beta_{\tau}\cdot H_{\tau}^{(l)}\cdot W_{\tau}^{(l)})
